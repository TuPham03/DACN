...
const CreatePost = ({ isEdit }) => {
  ...
  const [payload, setPayload] = useState(() => {
    const initData = {
      ...
      vipcost : dataEdit?.vipcost || 0,
      vipcost_type : dataEdit?.vipcost_type || 0,

      vipcost_day : dataEdit?.vipcost_day || 1,
    }
    return initData
  });
    ...
    if (result === 0) {
      if (dataEdit && isEdit) {
        ...
        if (response?.data.err === 0) {
          Swal.fire("Thành công", "Đã chỉnh sửa bài đăng thành công!", "success").then(() => {
            resetPayload()
            dispatch(resetDataEdit())
          });
        } else {
          Swal.fire("Thất bại~!", "Có lỗi khi đăng bài!", "error");
        }
      } else {
        ...
        if (response?.data.err === 0) {
          ...
          let type = response?.data.msg;
          let msg = "";
          if(type === "typed") msg = "Bạn vui lòng chọn gói ngày, tuần, tháng muốn đăng tin.";
          if(type === "day") msg ="Bạn vui lòng nhập thời gian muốn duy trì tin.";
          if(type === "package") msg ="Không thể tìm gói đăng tin, vui lòng chọn loại tin đăng phù hợp";
          if(type === "user") msg= "Vui lòng đăng nhập để đăng tin";
          if(type === "money") msg ="Tài khoản của bạn không đủ tiền để đăng tin";
          if(msg.length >=1) {
  ...
  return (
    <div className="px-6">
      <h1 className="text-3xl font-medium py-4 border-b border-gray-200">{isEdit ? 'Chỉnh sửa tin đăng' : 'Đăng tin mới'}</h1>
      <div className="flex gap-4 ">
        <div className=" flex flex-col gap-8 py-4 flex-auto">
          <Address
          ...
          <Button
            ...
            text={isEdit ? 'Cập nhật' : "Tạo mới"}
