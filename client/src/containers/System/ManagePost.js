...
const ManagePost = () => {
    ...
    useEffect(() => {
        if (status === 1) {
            const activePost = postOfCurrent?.filter(item => checkTimeDateNow(item?.vipCost_time))
            setPosts(activePost)
        } else if (status === 2) {
            const expiredPost = postOfCurrent?.filter(item => !checkTimeDateNow(item?.vipCost_time))
