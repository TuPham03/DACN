...
const Modal = ({ setIsShowMowdal, content, name, handleSubmit, queries, arrMinMax, defaultText }) => {
    ...
    const handleBeforeSubmid = (e) => {
        ...
        handleSubmit(e, {
            ...
            [name]: `Từ ${convert100toTarget(min)}${(persent1 === persent2 && persent1 === 100)
                ? ""
                : ` - ${convert100toTarget(max)}`} ${name === 'price' ? 'triệu' : 'm2'} ${(persent1 === persent2 && persent1 === 100) ? "+" : ""}`
        }, {
            [`${name}Arr`]: [min, max]
        })
    }
    ...
    const convert100toTarget = (percent) => {
        ...
        //10% -> 1.5
        //9% -> 1.35 * 10 = 14 / 5 = 2 du 4 => 3 * 5 = 15 / 10 = 1.5 => 1.5 trieu
        // 11% => 1.65 * 10 = 17/ 5 = 3 du 2 =? 4 * 5 = 20/ 10 = 2 -> 2 trieu
    ...
    return (
        <div onClick={(e) => { setIsShowMowdal(false) }}
                    ...
                    <div className='flex flex-col items-center justify-center relative'>
                        <div className='z-30 absolute top-[-48px] font-bold text-xl text-orange-600'>
                            {(persent1 === 100 && persent2 === 100)
                                ? `Trên ${convert100toTarget(persent1)} ${name === 'price' ? 'triệu' : 'm2'} +`
                                : `Từ ${persent1 <= persent2
                                    ? convert100toTarget(persent1)
                                    : convert100toTarget(persent2)} - ${persent2 >= persent1
                                        ? convert100toTarget(persent2) : convert100toTarget(persent1)} ${name === 'price'
                                            ? 'triệu'
                                            : 'm2'}`}
                        </div>
                        ...
                        <div className='absolute z-30 top-6 left-0 right-0 flex justify-between items-center'>
                            ...
                            >
                                {name === 'price' ? '15 triệu +' : name === 'area' ? 'Trên 90 m2' : ''}
                            </span>
                        </div>
                    </div>
                    <div className='mt-24'>
                        <h4 className='font-medium mb-6'>Chọn nhanh:</h4>
